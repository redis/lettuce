...
- Support for the `CELL_SIZE` option on [`CMS.INITBYDIM`](https://redis.io/docs/latest/commands/cms.initbydim/) and [`CMS.INITBYPROB`](https://redis.io/docs/latest/commands/cms.initbyprob/) via a `cellSize` overload of `cmsInitByDim`/`cmsInitByProb`, the `cell_size` field of [`CMS.INFO`](https://redis.io/docs/latest/commands/cms.info/) through `CMSInfoValue.getCellSize()`, and negative increments on [`CMS.INCRBY`](https://redis.io/docs/latest/commands/cms.incrby/) (Redis 8.12) — see [Probabilistic data structures](user-guide/probabilistic.md#count-min-sketch)
