...
- Support for the unified [`ZRANGE`](https://redis.io/docs/latest/commands/zrange/) syntax introduced in Redis 6.2 through new `zrange` and `zrangeWithScores` overloads accepting `ZRange` (an index or score `BYSCORE` range) together with `ZRangeArgs` (`REV`, `LIMIT`), plus `zrangeWithLex` overloads accepting a plain `Range` (a lexicographical `BYLEX` range) — `BYLEX` cannot be combined with `WITHSCORES`, so there is no `WITHSCORES` variant for lexicographical ranges. Available in the sync, async, reactive and Kotlin APIs — a single entry point covering the `ZREVRANGE`, `ZRANGEBYSCORE`, `ZREVRANGEBYSCORE`, `ZRANGEBYLEX` and `ZREVRANGEBYLEX` variants
