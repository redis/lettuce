...
/**
 * Cluster topology-change coverage for {@code HIMPORT}: a node that becomes the owner of a slot self-prepares a fieldset when a
 * {@code SET} first routes to it — its own outbound handler injects the {@code PREPARE}, so no broadcast or shared state is
 * needed. Both a newly joined master and a promoted replica are covered. Each is proven via a command listener: the {@code SET}
 * routed to the new owner succeeds with zero {@code no such fieldset} failures.
 */
...
public class HashImportClusterTopologyIntegrationTests extends TestSupport {
    ...
    /**
     * A brand-new master that joins the cluster and takes over a slot self-prepares the fieldset when a {@code SET} first
     * routes to it, with no explicit prepare and no shared state.
     */
    @Test
    void newMasterSelfPreparesFieldsetOnJoin() {
        ...
        try {
            ...
            // A brand-new master (port6) joins the cluster and takes over one slot.
            int slot = 16000;
    ...
    /**
     * A replica promoted to master serves {@code SET} by self-preparing the fieldset when a {@code SET} first routes to it,
     * just like a newly joined master.
     */
    @Test
