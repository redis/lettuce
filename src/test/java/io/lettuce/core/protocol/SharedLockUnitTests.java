...
public class SharedLockUnitTests {
    ...
    /**
     * When the calling thread's interrupt flag is already set and the lock experiences brief contention, {@code doExclusive}
     * must wait for contention to clear, succeed without throwing {@link RedisException}, and preserve the interrupted status
     * on the thread.
     */
    @Test
    @Timeout(5)
    public void doExclusiveSucceedsWhenThreadIsInterruptedAndLockIsContended() throws Exception {
        final SharedLock sharedLock = new SharedLock(Duration.ofSeconds(2));
        Lock internalLock = extractInternalLock(sharedLock);
        ...
        Assertions.assertTrue(held.await(1, TimeUnit.SECONDS), "holder failed to acquire lock");

        Thread.currentThread().interrupt();
        try {
            // Release after 50ms so contention clears before the 2s timeout
            new Thread(() -> {
                try {
                    Thread.sleep(50);
