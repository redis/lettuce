...
class MultiDbClientThreadLocalOptionsIntegrationTests {
        ...
        /**
         * Test that async connect with different ClientOptions per database properly applies options to each database
         * connection.
         */
        @Test
        ...
        /**
         * Test that verifies multiple parallel async connections each get the correct per-database options. This test ensures
         * that ThreadLocal options don't leak between different connection attempts.
         */
        @Test
         ...
         * pairs are completed before connectAsync() returns 3) Async callbacks run on a different thread (event loop) and have
         * no timing relationship with resetOptions() 4) Despite immediate reset, correct options are applied to each database
         */
        @Test
