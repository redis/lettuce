...
/**
 * Unit tests for {@link TsCreateArgs}.
 * <p>
 ...
 * <li>Given {@code ignore(maxTimeDiff, maxValDiff)}, when {@code build()}, then both values are emitted after
 * {@code IGNORE}.</li>
 * <li>Given every option combined, when {@code build()}, then tokens appear in the wire-contract order: RETENTION, ENCODING,
 * CHUNK_SIZE, DUPLICATE_POLICY, IGNORE, LABELS.</li>
 * </ul>
 */
@Tag(UNIT_TEST)
class TsCreateArgsUnitTests {

    private static String bulk(String value) {
        return "$" + value.getBytes(StandardCharsets.UTF_8).length + "\r\n" + value + "\r\n";
    }

    private static String encode(TsCreateArgs createArgs) {
    ...
    void shouldRenderNoArgs() {
        assertThat(encode(new TsCreateArgs())).isEmpty();
    }

    @Test
    void shouldRenderRetention() {
        assertThat(encode(TsCreateArgs.Builder.retention(1000))).isEqualTo(bulk("RETENTION") + bulk("1000"));
    }

    @Test
    void shouldRenderEncoding() {
        assertThat(encode(TsCreateArgs.Builder.encoding(TsEncodingFormat.COMPRESSED)))
                .isEqualTo(bulk("ENCODING") + bulk("COMPRESSED"));
    }

    @Test
    void shouldRenderChunkSize() {
        assertThat(encode(TsCreateArgs.Builder.chunkSize(4096))).isEqualTo(bulk("CHUNK_SIZE") + bulk("4096"));
    }

    @Test
    void shouldRenderDuplicatePolicy() {
        assertThat(encode(TsCreateArgs.Builder.duplicatePolicy(TsDuplicatePolicy.LAST)))
                .isEqualTo(bulk("DUPLICATE_POLICY") + bulk("LAST"));
    }

    @Test
    void shouldRenderIgnore() {
        assertThat(encode(TsCreateArgs.Builder.ignore(100, 5.5))).isEqualTo(bulk("IGNORE") + bulk("100") + bulk("5.5"));
    }

    @Test
    void shouldRenderSingleLabel() {
        assertThat(encode(TsCreateArgs.Builder.label("region", "us"))).isEqualTo(bulk("LABELS") + bulk("region") + bulk("us"));
    }

    @Test
    void shouldRenderChainedLabels() {
        assertThat(encode(TsCreateArgs.Builder.label("region", "us").label("env", "prod")))
    ...
    void shouldRenderLabelsFromMapPreservingOrder() {
        ...
        assertThat(encode(TsCreateArgs.Builder.labels(labels)))
                .isEqualTo(bulk("LABELS") + bulk("region") + bulk("us") + bulk("env") + bulk("prod"));
    }
    ...
    void shouldAppendChainedLabelAfterLabelsMap() {
        ...
        assertThat(encode(TsCreateArgs.Builder.labels(labels).label("env", "prod")))
                .isEqualTo(bulk("LABELS") + bulk("region") + bulk("us") + bulk("env") + bulk("prod"));
    }
    ...
    void shouldRenderFullCombinationInWireOrder() {
        ...
        TsCreateArgs createArgs = TsCreateArgs.Builder.retention(1000).encoding(TsEncodingFormat.UNCOMPRESSED).chunkSize(4096)
                .duplicatePolicy(TsDuplicatePolicy.LAST).ignore(100, 5.5).labels(labels);

        assertThat(encode(createArgs)).isEqualTo(bulk("RETENTION") + bulk("1000") + bulk("ENCODING") + bulk("UNCOMPRESSED")
                + bulk("CHUNK_SIZE") + bulk("4096") + bulk("DUPLICATE_POLICY") + bulk("LAST") + bulk("IGNORE") + bulk("100")
                + bulk("5.5") + bulk("LABELS") + bulk("region") + bulk("us"));
    }
