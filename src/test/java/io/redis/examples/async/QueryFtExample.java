...
public class QueryFtExample {
    ...
    public void run() {
        ...
        try (StatefulRedisConnection<String, String> connection = redisClient.connect()) {
            ...
            // STEP_START ft4
            CompletableFuture<SearchReply<String, String>> fuzzyResults = asyncCommands
                    .ftSearch("idx:bicycle", "%optamized%").thenApply(res -> {
                        res.getResults().stream().sorted((doc1, doc2) -> doc1.getId().compareTo(doc2.getId())).forEach(doc -> {
                            System.out.printf("ID: %s\n", doc.getId());
                        });
                        // >>> ID: bicycle:3
            ...
            CompletableFuture.allOf(descriptionResults, startsWithResults, endsWithResults, fuzzyResults, fuzzierResults).join();
        }
        finally {
