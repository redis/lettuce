...
public class HomeJsonExample {
    ...
    public void run() {
        ...
        try (StatefulRedisConnection<String, String> connection = redisClient.connect()) {
            ...
            // STEP_END
            // REMOVE_START
            asyncCommands.del("user:1", "user:2", "user:3").toCompletableFuture().join();
            searchCommands.ftDropindex("idx:users")
                .exceptionally(ex -> null) // Ignore errors if the index doesn't exist.
            ...
            JsonObject user1 = parser.createJsonObject()
                .put("name", parser.createJsonValue("\"Paul John\""))
                .put("email", parser.createJsonValue("\"paul.john@example.com\""))
                .put("age", parser.createJsonValue("42"))
                .put("city", parser.createJsonValue("\"London\""));

            JsonObject user2 = parser.createJsonObject()
                .put("name", parser.createJsonValue("\"Eden Zamir\""))
                .put("email", parser.createJsonValue("\"eden.zamir@example.com\""))
                .put("age", parser.createJsonValue("29"))
                .put("city", parser.createJsonValue("\"Tel Aviv\""));

            JsonObject user3 = parser.createJsonObject()
                .put("name", parser.createJsonValue("\"Paul Zamir\""))
                .put("email", parser.createJsonValue("\"paul.zamir@example.com\""))
                .put("age", parser.createJsonValue("35"))
                .put("city", parser.createJsonValue("\"Tel Aviv\""));
            // STEP_END
            ...
            List<FieldArgs<String>> schema = Arrays.asList(
                ...
                TextFieldArgs.<String>builder().name("$.email").as("email").build(),
            ...
            // STEP_START query2
            SearchArgs<String, String> query2Args = SearchArgs.<String, String>builder()
                .returnField("city", null)
                .build();
            CompletableFuture<SearchReply<String, String>> query2 = searchCommands.ftSearch("idx:users", "Paul", query2Args)
                .thenApply(res -> {
                    ...
                    }).sorted().toArray()).containsExactly(
                        "ID: user:1, City: London",
                        "ID: user:3, City: Tel Aviv"
                    );
                    // REMOVE_END
            ...
            AggregateArgs<String, String> aggArgs = AggregateArgs.<String, String>builder()
                .groupBy(GroupBy.<String, String>of("@city")
                ...
                .thenApply(res -> {
                    ...
                    replies.forEach(reply -> {
                        reply.getResults().forEach(result -> {
                            System.out.printf("City: %s, Count: %s\n", result.getFields().get("city"), result.getFields().get("count"));
                        });
                        // >>> City: London, Count: 1
                    ...
                    assertThat(replies.get(0).getResults().stream().map(result -> {
                        return String.format("City: %s, Count: %s", result.getFields().get("city"), result.getFields().get("count"));
                    }).sorted().toArray()).containsExactly(
                        "City: London, Count: 1",
                        "City: Tel Aviv, Count: 2"
                    );
