...
internal class RedisStringSuspendableReactiveCommands<K, V>(private val ops: RedisStringReactiveCommands<K, V>) : RedisStringSuspendableCommands<K, V> {
     ...
     * 
     **/
    override suspend fun mget(vararg keys: K?): List<KeyValue<K, V>>? = ops.mget(*keys).collectList().awaitFirstOrNull()
     ...
     * 
     **/
    override suspend fun mget(channel: KeyValueStreamingChannel<K, V>?, vararg keys: K?): Long? = ops.mget(channel, *keys).awaitFirstOrNull()
