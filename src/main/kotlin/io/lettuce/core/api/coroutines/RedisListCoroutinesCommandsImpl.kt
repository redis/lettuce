...
internal class RedisListCoroutinesCommandsImpl<K : Any, V : Any>(internal val ops: RedisListReactiveCommands<K, V>) : RedisListCoroutinesCommands<K, V> {
    ...
    override suspend fun lpop(key: K, count: Long): List<V> = ops.lpop(key, count).asFlow().toList()
    ...
    override suspend fun rpop(key: K, count: Long): List<V> = ops.rpop(key, count).asFlow().toList()
