...
internal class RedisCuckooFilterCoroutinesCommandsImpl<K : Any, V : Any>(
    ...
    override suspend fun cfInsert(key: K, value: V): List<Boolean?> =
        ops.cfInsert(key, value).asFlow().toList().map { it.getValueOrElse(null) }

    override suspend fun cfInsert(key: K, args: CfInsertArgs, value: V): List<Boolean?> =
        ops.cfInsert(key, args, value).asFlow().toList().map { it.getValueOrElse(null) }
    ...
    override suspend fun cfInsertNx(key: K, value: V): List<Boolean?> =
        ops.cfInsertNx(key, value).asFlow().toList().map { it.getValueOrElse(null) }

    override suspend fun cfInsertNx(key: K, args: CfInsertArgs, value: V): List<Boolean?> =
        ops.cfInsertNx(key, args, value).asFlow().toList().map { it.getValueOrElse(null) }
