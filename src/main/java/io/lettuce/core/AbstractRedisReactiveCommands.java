...
public abstract class AbstractRedisReactiveCommands<K, V>
    ...
    public Mono<String> himportPrepare(HashImport<K> fieldset) {
        ...
        return createMono(() -> {
            ...
            return commandBuilder.himportPrepare(fieldset);
        });
    }
    ...
    public Mono<String> himportSet(K key, HashImport<K> fieldset, V... values) {
        ...
        return Mono.defer(() -> {
            ...
            return createMono(() -> commandBuilder.himportSet(key, fieldset, values))
                    .onErrorResume(RedisCommandExecutionException.class, ex -> {
                        if (isNoSuchFieldset(ex)) {
                            return himportPrepare(fieldset)
                                    .then(createMono(() -> commandBuilder.himportSet(key, fieldset, values)));
                        }
    ...
    private static boolean isNoSuchFieldset(RedisCommandExecutionException ex) {
        return ex.getMessage() != null && ex.getMessage().toLowerCase().contains("no such fieldset");
    }
    ...
    @Override
    public Flux<Value<Boolean>> cfInsertNx(K key, V value) {
        return createDissolvingFlux(() -> cuckooFilterCommandBuilder.cfInsertNxValues(key, value));
    }

    @Override
    public Flux<Value<Boolean>> cfInsertNx(K key, CfInsertArgs args, V value) {
        return createDissolvingFlux(() -> cuckooFilterCommandBuilder.cfInsertNxValues(key, args, value));
    }
