...
public interface NodeSelectionListAsyncCommands<K, V> {
     ...
     * {@code source} is empty; with {@code EXACTLY} it blocks until {@code source} holds enough elements or the configured
     * timeout is reached. Without a count block this is equivalent to {@link #blmove(Object, Object, LMoveArgs, long)}. The
     * blocking timeout is carried by {@code args} via {@link BLMovemArgs#timeout(long)} / {@link BLMovemArgs#timeout(double)}
     * and defaults to {@code 0} (block indefinitely).
     *
     * @param source the source key.
     ...
     *         {@code EXACTLY} cannot be satisfied.
     * @since 7.7
     */
    AsyncExecutions<List<V>> blmovem(K source, K destination, BLMovemArgs args);
     ...
     */
    AsyncExecutions<V> lindex(K key, long index);
     ...
     *         was not found.
     */
    AsyncExecutions<Long> linsert(K key, boolean before, V pivot, V value);
     ...
     * elements or returns an empty list when {@code source} does not hold enough elements. Without a count block this is
     ...
     * @since 7.7
     */
    AsyncExecutions<List<V>> lmovem(K source, K destination, LMovemArgs args);
     ...
     * @since 5.3.2
     */
    AsyncExecutions<Long> lpos(K key, V value);
     ...
     * @since 5.3.2
     */
    AsyncExecutions<Long> lpos(K key, V value, LPosArgs args);
     ...
     * @since 5.3.2
     */
    AsyncExecutions<List<Long>> lpos(K key, V value, int count);
     ...
     * @since 5.3.2
     */
    AsyncExecutions<List<Long>> lpos(K key, V value, int count, LPosArgs args);
     ...
     */
    AsyncExecutions<Long> lpushx(K key, V... values);
     ...
     */
    AsyncExecutions<List<V>> lrange(K key, long start, long stop);
     ...
     */
    AsyncExecutions<Long> lrange(ValueStreamingChannel<V> channel, K key, long start, long stop);
     ...
     */
    AsyncExecutions<Long> lrem(K key, long count, V value);
     ...
     */
    AsyncExecutions<String> lset(K key, long index, V value);
     ...
     * @return String simple-string-reply.
     */
    AsyncExecutions<String> ltrim(K key, long start, long stop);
