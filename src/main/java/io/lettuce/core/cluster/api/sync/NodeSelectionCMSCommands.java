...
/**
 * Synchronous executed commands on a node selection for Count-Min Sketch.
 *
 ...
 * @since 7.7
 */
public interface NodeSelectionCMSCommands<K, V> {
     ...
     */
    Executions<List<Long>> cmsIncrBy(K key, IncrementPair<V> pair);
     ...
     */
    Executions<List<Long>> cmsIncrBy(K key, IncrementPair<V>... pairs);
     ...
     */
    Executions<CMSInfoValue> cmsInfo(K key);
     ...
     */
    Executions<String> cmsInitByDim(K key, long width, long depth);
     ...
     */
    Executions<String> cmsInitByProb(K key, double error, double probability);
     ...
     */
    Executions<String> cmsMerge(K destination, K source);
     ...
     * @return String simple-string-reply {@code OK} if {@code CMS.MERGE} was executed correctly.
     */
    Executions<String> cmsMerge(K destination, MergePair<K>... sources);
     ...
     */
    Executions<List<Long>> cmsQuery(K key, V value);
     ...
     */
    Executions<List<Long>> cmsQuery(K key, V... values);
