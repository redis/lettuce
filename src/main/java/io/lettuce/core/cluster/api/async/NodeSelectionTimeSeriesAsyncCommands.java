...
/**
 * Asynchronous executed commands on a node selection for RedisTimeSeries.
 *
 * @author Gyumin Hwang
 * @param <K> Key type.
 * @param <V> Value type.
 ...
 * @since 7.8
 */
public interface NodeSelectionTimeSeriesAsyncCommands<K, V> {
     ...
     */
    AsyncExecutions<String> tsCreate(K key);
     ...
     * @return String simple-string-reply {@code OK} if {@code TS.CREATE} was executed correctly.
     */
    AsyncExecutions<String> tsCreate(K key, TsCreateArgs createArgs);
     ...
     */
    AsyncExecutions<String> tsAlter(K key);
     ...
     * @return String simple-string-reply {@code OK} if {@code TS.ALTER} was executed correctly.
     */
    AsyncExecutions<String> tsAlter(K key, TsAlterArgs alterArgs);
     ...
     */
    AsyncExecutions<String> tsCreateRule(K sourceKey, K destKey, TsAggregationType aggregationType, long bucketDuration);
     ...
     */
    AsyncExecutions<String> tsDeleteRule(K sourceKey, K destKey);
     ...
     */
    AsyncExecutions<Long> tsDel(K key, long fromTimestamp, long toTimestamp);
     ...
     */
    AsyncExecutions<Long> tsAdd(K key, long timestamp, double value);
     ...
     * @return Long integer-reply the timestamp that was ultimately used for the sample.
     */
    AsyncExecutions<Long> tsAdd(K key, long timestamp, double value, TsAddArgs addArgs);
     ...
     * @return Long integer-reply the timestamp that was ultimately used for the sample.
     */
    AsyncExecutions<Long> tsAdd(K key, double value);
     ...
     * @return Long integer-reply the timestamp that was ultimately used for the sample.
     */
    AsyncExecutions<Long> tsAdd(K key, double value, TsAddArgs addArgs);
     ...
     */
    AsyncExecutions<List<Long>> tsMAdd(TsMAddValue<K>... entries);
     ...
     *         an entry that failed (e.g. a {@code DUPLICATE_POLICY=BLOCK} violation).
     */
    AsyncExecutions<List<Long>> tsMAdd(TsMAddValue<K> entry);
     ...
     */
    AsyncExecutions<Long> tsIncrBy(K key, double value);
     ...
     * @return Long integer-reply the timestamp of the updated sample.
     */
    AsyncExecutions<Long> tsIncrBy(K key, double value, TsIncrByArgs incrByArgs);
     ...
     * @return Long integer-reply the timestamp of the updated sample.
     */
    AsyncExecutions<Long> tsDecrBy(K key, double value);
     ...
     * @return Long integer-reply the timestamp of the updated sample.
     */
    AsyncExecutions<Long> tsDecrBy(K key, double value, TsIncrByArgs decrByArgs);
     ...
     */
    AsyncExecutions<TsSample> tsGet(K key);
     ...
     * @return TsSample the last sample of the time series, or {@code null} if the series has no samples.
     */
    AsyncExecutions<TsSample> tsGet(K key, boolean latest);
     ...
     */
    AsyncExecutions<List<TsMGetValue<K>>> tsMGet(TsFilter... filters);
     ...
     * @throws io.lettuce.core.RedisCommandExecutionException if no filter is an equality filter.
     */
    AsyncExecutions<List<TsMGetValue<K>>> tsMGet(TsFilter filter);
     ...
     * @throws io.lettuce.core.RedisCommandExecutionException if no filter is an equality filter.
     */
    AsyncExecutions<List<TsMGetValue<K>>> tsMGet(TsMGetArgs mGetArgs, TsFilter... filters);
     ...
     * @throws io.lettuce.core.RedisCommandExecutionException if no filter is an equality filter.
     */
    AsyncExecutions<List<TsMGetValue<K>>> tsMGet(TsMGetArgs mGetArgs, TsFilter filter);
     ...
     */
    AsyncExecutions<TsInfoValue<K>> tsInfo(K key);
     ...
     * @return TsInfoValue metadata about the time series, including chunk-level debug information if {@code debug} is
     *         {@code true}.
     */
    AsyncExecutions<TsInfoValue<K>> tsInfo(K key, boolean debug);
     ...
     * @throws io.lettuce.core.RedisCommandExecutionException if no filter is an equality filter.
     */
    AsyncExecutions<List<K>> tsQueryIndex(TsFilter... filters);
     ...
     * @throws io.lettuce.core.RedisCommandExecutionException if no filter is an equality filter.
     */
    AsyncExecutions<List<K>> tsQueryIndex(TsFilter filter);
