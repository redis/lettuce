...
/**
 * Asynchronous executed commands on a node selection for Top-K.
 *
 ...
 * @since 7.7
 */
public interface NodeSelectionTopKAsyncCommands<K, V> {
     ...
     */
    AsyncExecutions<List<String>> topKAdd(K key, V value);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    AsyncExecutions<List<String>> topKAdd(K key, V... values);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    AsyncExecutions<List<String>> topKIncrBy(K key, V value, long increment);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    AsyncExecutions<List<String>> topKIncrBy(K key, IncrementPair<V>... pairs);
     ...
     */
    AsyncExecutions<TopKInfoValue> topKInfo(K key);
     ...
     */
    AsyncExecutions<List<String>> topKList(K key);
     ...
     */
    AsyncExecutions<List<TopKListValue>> topKList(K key, boolean withCount);
     ...
     */
    AsyncExecutions<List<Boolean>> topKQuery(K key, V value);
     ...
     *         {@code false} otherwise.
     */
    AsyncExecutions<List<Boolean>> topKQuery(K key, V... values);
     ...
     */
    AsyncExecutions<String> topKReserve(K key, long k);
     ...
     * @return String simple-string-reply {@code OK} if the sketch was created.
     */
    AsyncExecutions<String> topKReserve(K key, long k, TopKReserveArgs args);
