...
class SharedLock {
     ...
     * <p>
     * An uncontended lock is acquired via {@link Lock#tryLock()} first so that callers with their interrupt flag already set
     * (such as during {@code close()}/{@code closeAsync()}) do not fail an otherwise successful exclusive operation solely due
     * to the interrupt.
     *
        ...
        if (!acquired) {
            ...
            if (timeoutNanos > 0) {
                try {
                    ...
                    throw new RedisException("Interrupted while acquiring the exclusive SharedLock", e);
                }
            }
        }
