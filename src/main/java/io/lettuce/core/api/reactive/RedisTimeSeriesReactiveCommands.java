...
public interface RedisTimeSeriesReactiveCommands<K, V> {
     ...
     * @return Long integer-reply the timestamp that was ultimately used for the sample.
     */
    Mono<Long> tsAdd(K key, double value, TsAddArgs addArgs);
     ...
     */
    Flux<Value<Long>> tsMAdd(TsMAddValue<K>... entries);
     ...
     *         {@link Value#empty()} for an entry that failed (e.g. a {@code DUPLICATE_POLICY=BLOCK} violation).
     */
    Flux<Value<Long>> tsMAdd(TsMAddValue<K> entry);
     ...
     * @return TsSample the last sample of the time series, or {@code null} if the series has no samples.
     */
    Mono<TsSample> tsGet(K key, boolean latest);
     ...
     *         {@code true}.
     */
    Mono<TsInfoValue<K>> tsInfo(K key, boolean debug);
