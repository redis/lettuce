...
/**
 * Asynchronous executed commands for Top-K.
 *
 ...
 * @since 7.7
 */
public interface RedisTopKAsyncCommands<K, V> {
     ...
     */
    RedisFuture<List<String>> topKAdd(K key, V value);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    RedisFuture<List<String>> topKAdd(K key, V... values);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    RedisFuture<List<String>> topKIncrBy(K key, V value, long increment);
     ...
     *         entered it, or {@code null} if no value was expelled.
     */
    RedisFuture<List<String>> topKIncrBy(K key, IncrementPair<V>... pairs);
     ...
     */
    RedisFuture<TopKInfoValue> topKInfo(K key);
     ...
     */
    RedisFuture<List<TopKListValue>> topKList(K key, boolean withCount);
     ...
     */
    RedisFuture<List<Boolean>> topKQuery(K key, V value);
     ...
     *         {@code false} otherwise.
     */
    RedisFuture<List<Boolean>> topKQuery(K key, V... values);
     ...
     * @return String simple-string-reply {@code OK} if the sketch was created.
     */
    RedisFuture<String> topKReserve(K key, long k, TopKReserveArgs args);
