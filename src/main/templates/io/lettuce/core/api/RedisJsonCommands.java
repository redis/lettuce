...
public interface RedisJsonCommands<K, V> {
     ...
     * @param elements one or more {@link JsonElement} to be appended.
     * @return Long the resulting size of the arrays after the new data was appended, or null if the path does not exist.
     * @since 6.5
     */
    List<Long> jsonArrappend(K key, JsonPath jsonPath, JsonElement... elements);

    /**
     * Search for the first occurrence of a {@link JsonElement} in an array at a given {@link JsonPath} and return its index.
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the array inside the document.
     * @param element the {@link JsonElement} to search for.
     ...
     * @since 6.5
     */
    List<Long> jsonArrindex(K key, JsonPath jsonPath, JsonElement element, JsonRangeArgs range);

    /**
     * Insert the {@link JsonElement}s into the array at a given {@link JsonPath} before the provided index, shifting the
     * existing elements to the right
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the array inside the document.
     * @param index the index before which the new elements will be inserted.
     * @param elements one or more {@link JsonElement}s to be inserted.
     * @return Long the resulting size of the arrays after the new data was inserted, or null if the path does not exist.
     * @since 6.5
     */
    List<Long> jsonArrinsert(K key, JsonPath jsonPath, int index, JsonElement... elements);
    ...
    /**
     * Remove and return {@link JsonElement} at a given index in the array at a given {@link JsonPath}
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the array inside the document.
     ...
     * @since 6.5
     */
    List<JsonElement> jsonArrpop(K key, JsonPath jsonPath, int index);
     ...
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element to clear.
     ...
     * @param jsonPaths the {@link JsonPath}s to use to identify the elements to get.
     * @return JsonElement the value at path in JSON serialized form, or null if the path does not exist.
     * @since 6.5
     */
    JsonElement jsonGet(K key, JsonGetArgs options, JsonPath... jsonPaths);

    /**
     * Merge a given {@link JsonElement} inside the JSON element matching {@link JsonPath}. Consequently, JSON values at
     ...
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element to merge.
     * @param element the {@link JsonElement} to merge.
     * @return Boolean true if the merge was successful, false otherwise.
     * @since 6.5
     * @see <A href="https://tools.ietf.org/html/rfc7396">RFC7396</a>
     */
    Boolean jsonMerge(K key, JsonPath jsonPath, JsonElement element);

    /**
     * Return the values at path from multiple key arguments.
     *
     * @param jsonPath the {@link JsonPath} pointing to the element to fetch.
     * @param keys the keys holding the {@link JsonElement}s to fetch.
     * @return List<JsonElement> the values at path, or null if the path does not exist.
     * @since 6.5
     */
    List<JsonElement> jsonMGet(JsonPath jsonPath, K... keys);
     ...
     * @since 6.5
     */
    Boolean jsonMSet(JsonMsetArgs... arguments);

    /**
     * Increment the number value stored at the specified {@link JsonPath} in the JSON document by the provided increment.
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element to increment.
     * @param number the increment value.
     * @return an array of the new values after the increment.
     * @since 6.5
     */
    JsonElement jsonNumincrby(K key, JsonPath jsonPath, Number number);

    /**
     * Return the keys in the JSON document that are referenced by the given {@link JsonPath}
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element(s) whose key(s) we want.
     * @return List<V> the keys in the JSON document that are referenced by the given {@link JsonPath}.
     * @since 6.5
     */
    List<List<V>> jsonObjkeys(K key, JsonPath jsonPath);

    /**
     * Report the number of keys in the JSON object at path in key
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element(s) whose key(s) we want to count
     ...
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element(s) where we want to set the value.
     * @param element the {@link JsonElement} to set.
     * @param options the {@link JsonSetArgs} the options for setting the element.
     * @return Boolean true if the set was successful, false otherwise, null if the {@link JsonSetArgs} conditions are not met.
     * @since 6.5
     */
    Boolean jsonSet(K key, JsonPath jsonPath, JsonElement element, JsonSetArgs options);

    /**
     * Append the json-string values to the string at the provided {@link JsonPath} in the JSON document.
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element(s) where we want to append the value.
     * @param element the {@link JsonElement} to append.
     * @return Long the new length of the string, or null if the matching JSON value is not a string.
     * @since 6.5
     */
    List<Long> jsonStrappend(K key, JsonPath jsonPath, JsonElement element);

    /**
     * Report the length of the JSON String at the provided {@link JsonPath} in the JSON document.
     *
     * @param key the key holding the JSON document.
     * @param jsonPath the {@link JsonPath} pointing to the element(s).
     * @return Long (in recursive descent) the length of the JSON String at the provided {@link JsonPath}, or null if the
     ...
     * @since 6.5
     */
    List<Boolean> jsonToggle(K key, JsonPath jsonPath);
