...
- **Methods**: start with an **imperative verb** ("Append", "Get", "Remove", "Create") — this
  matches the Redis command reference and the existing command interfaces, which are overwhelmingly
  imperative. Not "This method…". (Third person is acceptable for non-command API, e.g. factory
  methods, when it matches the surrounding file — but imperative is the default for command methods.)
...
```java
// GOOD (method)   Append a value to a key.
// GOOD (class)    Redis URI holding connection details for Redis and Sentinel connections.
// BAD             This method appends a value to a key.            // "This method" / weak
// BAD             append.                                          // restates the name, no period
```
...
- Write in **US English**, present tense. Method first-sentence summaries are imperative (§3);
  the rest of the description is plain declarative prose.
...
- Imperative verb summary (§3). Document parameters, return, exceptions, side effects, and threading.
